...
RSpec.describe "Structured-query contracts" do
  let(:contract_path) { File.expand_path("../fixtures/structured_query/query_ir_v2.json", __dir__) }
  ...
  it "fixes the stable Query IR v2 serialization vocabulary" do
  ...
  it "defines plan and result statuses independently from result kinds" do
    ...
    expect(contract.fetch("result_kinds")).to eq(%w[relation scalar aggregate table rag_answer hybrid_answer none])
  end
